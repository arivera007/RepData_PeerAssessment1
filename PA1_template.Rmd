...
#myFilledIntervalMean <- aggregate(myActivity$steps, list(interval = myActivity$interval), mean, na.rm=TRUE)
#qplot(myFilledActivity$interval, myFilledActivity$x, geom = "line")

myPanels <- ggplot(myFilledActivity, aes(interval, steps)) + facet_grid(x ~ .) +stat_summary(fun.y = mean, geom = "line")    #+ geom_line()
#myPanels + facet_grid(x ~ .)
